...
RSpec.describe 'all merchant biz intel' do
  ...
  it "returns x number of merchants based on quantity sold" do
    merchant1 = Merchant.create!(name: "Kermit")
    merchant2 = Merchant.create!(name: "Fozzy")
    merchant3 = Merchant.create!(name: "Miss Piggy")
    item1 = Item.create!(name: "frog", description: "green", unit_price: 2345, merchant_id: merchant1.id)
    ...
    expect(response).to be_successful
    expect(results["data"][0]["attributes"]["id"]).to eq(merchant2.id)
    expect(results["data"][0]["attributes"]["name"]).to eq(merchant2.name)
    expect(results["data"][1]["attributes"]["id"]).to eq(merchant3.id)
    expect(results["data"][1]["attributes"]["name"]).to eq(merchant3.name)
  end

  it "returns total revenue of all merchants for a date" do
    merchant1 = Merchant.create!(name: "Kermit")
    merchant2 = Merchant.create!(name: "Fozzy")
    merchant3 = Merchant.create!(name: "Miss Piggy")
    item1 = Item.create!(name: "frog", description: "green", unit_price: 2345, merchant_id: merchant1.id)
    ...
    item3 = Item.create!(name: "pig", description: "pink", unit_price: 3344, merchant_id: merchant3.id)
    customer1 = Customer.create!(first_name: "Jim", last_name: "Henson")
    invoice1 = Invoice.create!(status: "shipped", customer_id: customer1.id, merchant_id: merchant1.id, created_at: "2019-06-25 09:54:09 UTC")
    invoice2 = Invoice.create!(status: "shipped", customer_id: customer1.id, merchant_id: merchant2.id, created_at: "2019-06-25 09:54:09 UTC")
    invoice3 = Invoice.create!(status: "shipped", customer_id: customer1.id, merchant_id: merchant3.id, created_at: "2019-06-25 09:54:09 UTC")
    invoice4 = Invoice.create!(status: "shipped", customer_id: customer1.id, merchant_id: merchant1.id, created_at: "2019-06-25 09:54:09 UTC")
    invoice5 = Invoice.create!(status: "shipped", customer_id: customer1.id, merchant_id: merchant2.id, created_at: "2019-05-25 09:54:09 UTC")
    invoice6 = Invoice.create!(status: "shipped", customer_id: customer1.id, merchant_id: merchant3.id, created_at: "2019-06-25 09:54:09 UTC")
    ii1 = InvoiceItem.create!(quantity: 45, unit_price: 100, item_id: item1.id, invoice_id: invoice1.id)
    ii2 = InvoiceItem.create!(quantity: 35, unit_price: 10, item_id: item2.id, invoice_id: invoice2.id)
    ii3 = InvoiceItem.create!(quantity: 30, unit_price: 50, item_id: item3.id, invoice_id: invoice3.id)
    ii3 = InvoiceItem.create!(quantity: 20, unit_price: 12, item_id: item1.id, invoice_id: invoice4.id)
    ...
    get "/api/v1/merchants/revenue?date=2019-06-25"

    results = JSON.parse(response.body)

    expect(response).to be_successful
    expect(results["data"]["attributes"]["total_revenue"]).to eq("106.70")
  end
end
